...
def drive_rover(drive, servo, fwd, turn):
    drive.dev.duty_cycle = drive.mid_val + (drive.range_val * (fwd + turn) *.5)
    servo.dev.duty_cycle = servo.mid_val - (servo.range_val * (fwd - turn) *.5)

def hd_init():
    drive = pwm_dev(pwmio.PWMOut(board.D13), 0xB700, 0x4600)
    servo = pwm_dev(pwmio.PWMOut(board.D12), 0xB700, 0x4600)
...
async def rover_main(websocket, drive, servo):
    ...
    while True:
        ...
        websocket.reader.buffer = bytearray()
...
async def test():
    ...
    async for websocket in websockets.connect('ws://192.168.99.36:8000', max_queue=1024):
        #await test2(websocket, drive, servo)
        try:
            await asyncio.gather(rover_main(websocket, drive, servo))
...
async def test_cam():
    ...
    async for websocket in websockets.connect('ws://192.168.99.36:8001'):
        try:
            while True:
                ret, frame = cam.read()
                gray = frame
